...
export function ErrorsModal({open, sessionId, onClose}: ErrorsModalProps) {
  ...
  useEffect(() => {
    ...
    getCorrections(sessionId)
      ...
      .catch(() => {
        if (!cancelled) setError(t('chatError', {error: ''}))
      })
      .finally(() => {
  ...
  function downloadPdf(corrs: Correction[], sort: SortBy) {
    const doc = new jsPDF()
    doc.setFontSize(16)
    doc.text('All Corrections — AI Coach (Demo)', 14, 18)
    doc.setFontSize(10)
    doc.text(`Generated: ${new Date().toISOString().split('T')[0]}`, 14, 25)
    doc.text(`Total: ${corrs.length} corrections`, 14, 31)

    const grouped = groupCorrections(corrs, sort)
    let y = 40
    for (const [, list] of Object.entries(grouped)) {
      ...
      autoTable(doc, {
        head: [[
          t('correctionOriginal'),
          t('correctionCorrected'),
          t('correctionExplanation'),
          t('correctionCategory'),
        ]],
        body: list.map((c) => [
          c.original_phrase,
          c.corrected_phrase,
    ...
    >
      <div className={styles.modal}>
        <div className={styles.header}>
          <h2 className={styles.title}>{t('allCorrections')}</h2>
          <div className={styles.headerRight}>
            ...
            >
              <option value="level">{t('correctionLevel')} (high → low)</option>
        ...
        <div className={styles.body}>
          ...
          {!loading && !error && corrections.length > 0 && (
            <table className={styles.table}>
              <thead>
                <tr>
                  <th>{t('correctionOriginal')}</th>
                  <th>{t('correctionCorrected')}</th>
                  <th>{t('correctionExplanation')}</th>
                  <th>{t('correctionLevel')}</th>
                  <th>{t('correctionCategory')}</th>
                </tr>
              </thead>
              <tbody>
                {Object.values(grouped).flat().map((c) => (
                  <tr
                    key={c.id ?? `${c.original_phrase}-${c.timestamp}`}
